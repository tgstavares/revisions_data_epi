...
dat11= "<awk -F: '{if($1 <= 61){print $0}}' data/exportfinal_002.csv"
dat2 = "data/exportfinal_132.csv"
dat22= "<awk -F: '{if($1 <= 61){print $0}}' data/exportfinal_132.csv"
...
#set logscale y
set xrange ["2020-04-01":"2020-08-18"]
set title "Cumulative deaths by SARs-CoV-2 in Mexico"
plot dat1 index 0 u 4:($1<=61  ? $5 : 1/0) pt 65 ps 1.0 lw 3 lc 'dark-plum' title "total deaths (data no rev)", \
     dat2 index 0 u 4:($1<=61  ? $6 : 1/0) pt 68 ps 1.0 lw 3 lc 'dark-green' title "total deaths (data with rev)", \
     dat1 index 0 u 4:($1<=160 ? $3 : 1/0) w l lw 5 lc 'dark-plum' title "total deaths (model no rev)", \
...
plot dat11 index 0 u ($1+0.2):(diff(1,$5)) with impulses lw 2.0 lc 'dark-plum' title "daily deaths (data no rev)", \
     dat22 index 0 u ($1-0.2):(diff(1,$6)) with impulses lw 2.0 lc 'dark-green' dt (1,1) title "daily deaths (data with rev)", \
     dat1 index 0 u 1:($1<=160 ? diff(1,$3) : NaN) w l lw 5 lc 'dark-plum' title "daily deaths (model no rev)", \
